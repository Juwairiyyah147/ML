...
dt_phish = dataset[dataset.type=='phishing']
dt_malware = dataset[dataset.type=='malware']
dt_deface = dataset[dataset.type=='defacement']
dt_benign = dataset[dataset.type=='benign']
phish_url = " ".join(i for i in dt_phish.url)
...
def prefix_suffix(url):
    ...
    if(domain.count('-')):
        ...
        return -1
dataset['prefix_suffix'] = dataset['url'].apply(lambda i: prefix_suffix(i))
...
def sub_domain(url):
    ...
    if(subDomain.count('.')==0):
        ...
        return 1
dataset['sub_domain'] = dataset['url'].apply(lambda i: sub_domain(i))
...
def https_token(url):
    ...
    if(host.count('https')): #attacker can trick by putting https in domain part
        ...
        return -1
dataset['https_token'] = dataset['url'].apply(lambda i: https_token(i))
...
def age_of_domain(url):
    try:
        ...
        #op = '%s/%s/%s' % (start_date.month, start_date.day, start_date.year)
        current_date = datetime.datetime.now()
        #op1 = '%s/%s/%s' % (current_date.month, current_date.day, current_date.year)
...
#dataset['age_of_domain'] = dataset['url'].apply(lambda i: age_of_domain(i))
...
sns.set(style="darkgrid")
ax = sns.catplot(x="type",y='count.', kind="box",data=dataset)
